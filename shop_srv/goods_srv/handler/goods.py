    ...
    def GoodsList(self, request: goods_pb2.GoodsFilterRequest, context):
        ...
        if request.topCategory:
            ...
            try:
                ...
                # 无论是几级分类，均有id
                if level == 1:
                    # SQL: select * from category where parent_category_id in(select category.id from category where parent_category_id = ?)
                    c2 = Category.alias()
